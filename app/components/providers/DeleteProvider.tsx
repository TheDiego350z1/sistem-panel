...
export default function DeleteProvider({
  ...
  const onDelete = async () => {
    try {
      const { data } = await ConexionApi.delete(`/providers/${provider.id}`, {
        headers: {
      ...
      navigate(`/providers?page=${currentPage || 1}`);
    } catch (error) {
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4">
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            ...
            <p className="font-semibold text-red-900 mt-1">{provider.name}</p>
            <p className="text-sm text-red-700">{provider.email}</p>
          </div>
